...
class Hardware(models.Model):
    ...
    parent_serial_number = models.ForeignKey('self', to_field='serial_number', 
        blank = True, null = True, )
    project_name = models.CharField( max_length = 100, )
    comment = models.CharField( max_length = 140, blank = True, )

    def __unicode__(self):
        return 'eITMS: %s - Model: %s - S/N: %s - Project: %s' % 
                (self.eITMS_code, self.model_code, self.serial_number,self.project.name)
...
class User(models.Model):
    ...
    def __unicode__(self):
        return 'Name: %s - Email: %s - Company: %s' % 
                (self.name, self.email, self.company)
...
class Proj_Has_Users(models.Model):
    ...
    def __unicode__(self):
        return 'Project: %s - User: %s / %s' % 
                (self.project.name, self.user.name, self.user.email)
